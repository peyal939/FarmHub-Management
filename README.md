...
- Consistent response envelope for explicit create/update (farms, livestock, milk records)
 - Health endpoints: core (`/healthz`), reporting (`/health`, `/health/db`)

Prereqs (local): Python 3.12+, Docker 24+, Docker Compose plugin.

Environment variables: POSTGRES_* are used by the database container; DB_* are read by Django & FastAPI via `python-decouple`. They duplicate intentionally so app code is decoupled from raw container variable names (can be unified later).

## 3. Roles & Access (Summary Table)
| Resource | SUPERADMIN | AGENT | FARMER |
|----------|------------|-------|--------|
| Users    | Full       | –     | Read self |
...
Health checks:
- Core: http://localhost:8000/healthz/
- Reporting: http://localhost:8001/health (and /health/db)
...
Header: `Authorization: Bearer <access>` (sample credentials are development-only; change in any deployed environment.)
Example (PowerShell):
```powershell
...
```bash
curl "http://localhost:8001/reports/farm/1/daily-milk?start_date=2025-08-01&end_date=2025-08-23"
curl "http://localhost:8001/reports/farmer/5/summary?start_date=2025-08-01&end_date=2025-08-23"
```
...
Idempotent: migration checks for existing objects and will not duplicate.
...
Variables (defaults): `core_url` = http://localhost:8000, `reporting_url` = http://localhost:8001.
