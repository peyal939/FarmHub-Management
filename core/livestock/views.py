...
class CowViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):  # type: ignore[override]
        """Custom update with RBAC checks & normalized response envelope."""
        from rest_framework.response import Response
        from rest_framework.exceptions import PermissionDenied, ValidationError
        ...
        if Roles and role == Roles.AGENT:
            ...
            if (
                farm_id
                and owner_id
                and not FarmerProfile.objects.filter(id=owner_id, farm_id=farm_id).exists()
            ):
        ...
        if Roles and role == Roles.FARMER:
            ...
            serializer = self.get_serializer(instance, data=request.data, partial=partial)
            serializer.is_valid(raise_exception=True)
            # Force owner to self if trying to change to someone else
